...
from .workflow import TrackerConfig, WorkspaceConfig
...
class WorkspaceManager:
    def __init__(self, tracker_cfg: TrackerConfig, workspace_cfg: WorkspaceConfig) -> None:
        self._tracker = tracker_cfg
        self._root = Path(workspace_cfg.root).expanduser().resolve()
    ...
    def _path_for(self, identifier: str) -> tuple[Path, str]:
        ...
        # Invariant 2: path must stay under root.
    ...
    async def cleanup(self, issue: IssueRecord) -> None:
        ...
        if not path.exists():
            return
        # rmtree off the event loop
        await asyncio.get_running_loop().run_in_executor(None, shutil.rmtree, str(path), True)
    ...
    async def _git_clone(self, dest: Path) -> None:
        ...
        url = f"https://x-access-token:{token}@github.com/{self._tracker.owner}/{self._tracker.repo}.git"
        dest.parent.mkdir(parents=True, exist_ok=True)
        await self._run_git("clone", "--depth=50", url, str(dest), cwd=dest.parent)
        # Stage a sane git identity locally so commits the agent makes are valid.
    ...
    async def _checkout_branch(self, repo: Path, branch: str) -> None:
        ...
        # If the agent already pushed this branch on a prior run (the rework
        # path), reset the local branch to origin/{branch} so previous commits
        # are preserved instead of being thrown away.
        ...
        # First attempt for this issue — branch off the remote's default branch.
        for default_ref in ("origin/main", "origin/master"):
            ...
            if ok:
                return
        # No remote default found — create the branch off whatever HEAD is.
    ...
    async def _run_git(
        ...
        if not ok and check:
            raise WorkspaceError(
                f"git {args[0]} failed (exit {proc.returncode}): {err.decode(errors='replace')[:500]}"
            )
